...
if (!function_exists('amJL_certification')){
    function amJL_certification(){
        ...
        if(filter_input(INPUT_POST, 'main') === 'certification'){
            $plugin = sanitize_text_field(filter_input(INPUT_POST, 'plugin'));
            if (isset($plugin) && $plugin !== '') update_option('plugin', $plugin);
        }

        $password = sanitize_text_field(get_option('plugin'));

        $check_result = (bool)amJL_check_certification($password);

        if($check_result === true){
            require_once( amJL_Details );
        }else{
