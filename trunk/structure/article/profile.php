...
if (!function_exists('amJL_profile')){
    function amJL_profile(){
      ...
      $amJL_logo["dataModified"] = get_the_modified_date(DATE_ISO8601);
      ...
      $permalink = ( isset($https) && $htpps !== '' ? "http://" : "https://") .sanitize_text_field(filter_input(INPUT_SERVER, 'HTTP_HOST')) . sanitize_text_field(filter_input(INPUT_SERVER, 'REQUEST_URI'));
  
      if($profile_check === 1 && $permalink === $url1){
        echo "<!-- Profile page -->\n";
        echo '<script type="application/ld+json">'."\n";
        echo json_encode($amJL_logo,  JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE);
        echo "\n".'</script>'."\n";
      }
    }
  }
